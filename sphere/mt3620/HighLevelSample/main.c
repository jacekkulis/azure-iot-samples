...
static void ButtonPollTimerEventHandler(EventLoopTimer *timer)
{
    ...
    if (sendTelemetryButtonA || sendTelemetryButtonB) {
        ...
        if (sendTelemetryButtonA) {
            ...
            snprintf(pjsonBuffer, JSON_BUFFER_SIZE, cstrDeviceTwinJsonInteger, "buttonA",
                     !buttonAState);
        }

        if (sendTelemetryButtonB) {
            // construct the telemetry message for Button B
            snprintf(pjsonBuffer, JSON_BUFFER_SIZE, cstrDeviceTwinJsonInteger, "buttonB",
                     !buttonBState);
        }
...
static void ReadSensorTimerEventHandler(EventLoopTimer *timer)
{
    ...
    if (IoTCConnected) {
        ...
        // Keep track of the first time through this code.  The LSMD6S0 returns bad data the first
        // time we read it.  Don't send that data up in case we're charting the data.
...
static ExitCode InitPeripheralsAndHandlers(void)
{
    ...
    oledUpdateTimer =
        CreateEventLoopPeriodicTimer(eventLoop, &UpdateOledEventHandler, &oledUpdatePeriod);
    ...
    sensorPollTimer =
        CreateEventLoopPeriodicTimer(eventLoop, &ReadSensorTimerEventHandler, &readSensorPeriod);
    ...
    if (sockFd == -1) {
        ...
        rtAppEventReg =
            EventLoop_RegisterIo(eventLoop, sockFd, EventLoop_Input, SocketEventHandler, NULL);
        if (rtAppEventReg == NULL) {
            return ExitCode_Init_RegisterIo;
        }

        // Register one second timer to send a message to the real-time core.
        static const struct timespec M4PollPeriod = {.tv_sec = M4_READ_PERIOD_SECONDS,
                                                     .tv_nsec = M4_READ_PERIOD_NANO_SECONDS};
...
static bool ProvisionWithDpsPnP(void)
{
    ...
    //    #define IOT_PLUG_AND_PLAY_MODEL_ID "dtmi:com:example:azuresphere:avnetoob;1"	//
    ...
    if (_deviceTwinModelId != NULL && strlen(_deviceTwinModelId) > 0) {
        size_t modelIdLen = 20; // allow for JSON format "{\"modelId\":\"%s\"}", 14 char, plus null
    ...
    if ((prov_result = Prov_Device_LL_SetOption(
             prov_handle, "SetDeviceId", &deviceIdForDaaCertUsage)) != PROV_DEVICE_RESULT_OK) {
    ...
    if (dtdlBuffer != NULL) {
        if ((prov_result = Prov_Device_LL_Set_Provisioning_Payload(prov_handle, dtdlBuffer)) !=
    ...
    if ((prov_result = Prov_Device_LL_Register_Device(prov_handle, RegisterDeviceCallback, NULL,
                                                      NULL, NULL)) != PROV_DEVICE_RESULT_OK) {
    ...
    if ((iothubClientHandle = IoTHubDeviceClient_LL_CreateWithAzureSphereFromDeviceAuth(
             iotHubUri, MQTT_Protocol)) == NULL) {
    ...
    int deviceId = 1;
    if (IoTHubDeviceClient_LL_SetOption(iothubClientHandle, "SetDeviceId", &deviceId) !=
        IOTHUB_CLIENT_OK) {
        IoTHubDeviceClient_LL_Destroy(iothubClientHandle);
    ...
    if (dtdlBuffer != NULL) {
        if (IoTHubDeviceClient_LL_SetOption(iothubClientHandle, OPTION_MODEL_ID,
                                            _deviceTwinModelId) != IOTHUB_CLIENT_OK) {
...
static void ReadWifiConfig(bool outputDebug)
{
    ...
    if (result < 0) {
        ...
        snprintf(bssid, JSON_BUFFER_SIZE, "%02x:%02x:%02x:%02x:%02x:%02x", network.bssid[0],
                 network.bssid[1], network.bssid[2], network.bssid[3], network.bssid[4],
                 network.bssid[5]);

        // Check to see if the SSID changed, if so update the SSID and send updated device twins
        // properties
        ...
        if ((iothubClientHandle != NULL) && (ssidChanged)) {
            // Note that we send up this data to Azure if it changes, but the IoT Central Properties
            // elements only show the data that was currenet when the device first connected to
            // Azure.
