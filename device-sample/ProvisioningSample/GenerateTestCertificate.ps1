...
Param(
	$deviceName = "device-stream",
	$certificateValidityInYears = 1
)

$cert = New-SelfSignedCertificate -Type Custom -Subject "CN=$deviceName, O=PSC, C=PL" -KeySpec Signature -KeyExportPolicy Exportable -HashAlgorithm sha256 -KeyLength 2048 -TextExtension @("2.5.29.37={text}1.3.6.1.5.5.7.3.2") -CertStoreLocation "Cert:\CurrentUser\My" -NotAfter (Get-Date).AddYears($certificateValidityInYears)
